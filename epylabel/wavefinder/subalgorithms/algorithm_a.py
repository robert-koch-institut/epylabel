...
def delete_pairs(data: DataFrame, t_sep_a: int) -> DataFrame:
    ...
    if np.nanmin(data["duration"]) < t_sep_a and len(data) >= 3:
        ...
        # earlier.
        if (
            is_peak * (data.loc[i + 1, "y_position"] - data.loc[i - 1, "y_position"])
            >= 0
        ):
...
def run(
    input_data_df: DataFrame, prominence_updater: ProminenceUpdater, t_sep_a: int
) -> DataFrame:
    """
    ...
    if len(df) < 3:
        ...
        df["duration"] = df["location"].diff(periods=1) - df["location"].diff(
            periods=-1
        )
