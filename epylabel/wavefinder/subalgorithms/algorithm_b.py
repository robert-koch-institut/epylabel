...
def run(
    ...
    prominence_updater: ProminenceUpdater,
    t_sep_a: int,
) -> DataFrame:
    """
    ...
    while sub_b_flag:
        ...
        df.loc[0 : len(df) - 2, "y_distance"] = [
            abs(x) for x in np.diff(df["y_position"])
        ]
        ...
        for x in df.index:
            if df.loc[x, "separation"] < t_sep_a / 2:
                ...
                t_1 = min(
                    np.floor((original_t_0 + original_t_1 + t_sep_a) / 2),
                    raw_data.index[-1],
                )
    ...
    for key, val in original_data.items():
        df.loc[df["location"] == key, ["y_position", "location"]] = [
            val["y_position"],
            val["location"],
        ]
